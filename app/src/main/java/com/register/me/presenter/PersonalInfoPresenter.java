 ...
 */
public class PersonalInfoPresenter implements NetworkCall.NetworkGetUserInfoInterface, NetworkCall.NetworkUpdateUserInterface, Utils.UtilAlertInterface {
    ...
    public void onUserDataFetched(GetUserInfoModel body) {
        ...
        info.add(new QandA("First Name", firstName, 1,1,1,"firstname",null, null));
        info.add(new QandA("Last Name", lastName, 1,1,1,"lastname",null, null));
    ...
    public void updateUser(ArrayList<QandA> info) {
        JsonObject data = builder.getUserUpdateJson(info);
        Log.d("data", data.toString());
        apiCall(data);
    }

    public void apiCall(JsonObject data) {
        if(utils.isOnline(context)){
        ApiInterface apiInterface = retrofit.create(ApiInterface.class);
        String token = repo.getData(constants.getCACHE_TOKEN());
    ...
    @Override
    public void onUserUpdateFailed(String s) {
